...
function ManageCurriculumPage() {
  ...
  const handleCreateSemester = async (e) => {
    ...
    // Check for duplicate card type with same number
    if (newSemester.semester_number) {
      const duplicate = semesters.find(
        sem => sem.card_type === newSemester.card_type && 
               sem.semester_number === newSemester.semester_number
      )
      if (duplicate) {
        alert(`A ${newSemester.card_type} card with number ${newSemester.semester_number} already exists!`)
        return
      }
    }
  ...
  const handleUpdateSemester = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/semester/${editingSemester.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteSemester = async (semId, semName) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/semester/${semId}`, {
        method: 'DELETE',
      })
  ...
  const handleDeleteHonourCard = async (cardId, cardTitle) => {
    if (!window.confirm(`Are you sure you want to delete honour card "${cardTitle}"? This will also delete all verticals and courses within it. This action cannot be undone.`)) {
      return
    }

    try {
      const response = await fetch(`${API_BASE_URL}/honour-card/${cardId}`, {
        method: 'DELETE',
      })

      if (!response.ok) {
        throw new Error('Failed to delete honour card')
      }

      fetchHonourCards()
    } catch (err) {
      console.error('Error deleting honour card:', err)
      setError('Failed to delete honour card')
    }
  }
  ...
  return (
    <MainLayout 
      ...
      subtitle={`Semesters for Curriculum ID: ${id}`}
      actions={
          ...
          <div className="card-custom p-6">
            ...
            {cardType === 'normal' ? (
              <form onSubmit={handleCreateSemester} className="flex gap-4 items-end">
                ...
                {(newSemester.card_type === 'semester' || newSemester.card_type === 'vertical') && (
                  <div className="w-32">
            ...
            {semesters.map(sem => {
              ...
              return (
                ...
                >
                  ...
                  {(sem.card_type === 'semester' || sem.card_type === 'vertical') && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      const displayName = sem.semester_number 
                        ? `${cardType.charAt(0).toUpperCase() + cardType.slice(1)} ${sem.semester_number}` 
                        : cardType.charAt(0).toUpperCase() + cardType.slice(1)
                      handleDeleteSemester(sem.id, displayName)
                    }}
              ...
              <div className="bg-gradient-to-r from-green-600 to-green-700 text-white px-8 py-5 flex items-center justify-between rounded-t-2xl">
                <div>
                  ...
                  <p className="text-sm text-green-100">
                    {editingSemester.card_type === 'vertical' ? 'Update vertical number' : 'Update semester number'}
                  </p>
                </div>
                <button 
              ...
              <form onSubmit={handleUpdateSemester} className="p-8 space-y-5">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    {editingSemester.card_type === 'vertical' ? 'Vertical Number' : 'Semester Number'}
                  </label>
                  <input
                    type="number"
