      ...
      <div className="space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item, index) => {
            ...
            // Items from another department (source_department_id > 0)
              ...
              <div key={index} className="flex flex-col gap-2">
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      title={isFromOtherDept ? 'Shared from another department - editing will make your own copy' : isShared ? 'Shared with cluster - editing will make it private' : ''}
                    />
                  </div>
                  <button
                    type="button"
                ...
                {(isShared || isFromOtherDept) && (
                  <div className="ml-11 flex items-center gap-2">
                    {isShared && !isFromOtherDept && (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-md border border-green-200">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                        </svg>
                        Shared with cluster - Edit to unshare
                      </span>
                    )}
                    {isFromOtherDept && (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-100 text-blue-700 text-xs font-medium rounded-md border border-blue-200">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                        </svg>
                        From cluster - Edit to create own copy
                      </span>
                    )}
                  </div>
                )}
