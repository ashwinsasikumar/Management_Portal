...
function SharingManagementPage() {
  ...
  const handleConfirmSharing = async () => {
    if (currentSharingItem) {
      ...
      const itemToShare = currentSharingItem
      const depsToShare = selectedDepartments.length > 0 ? selectedDepartments : null
      ...
      handleToggleVisibility(
        itemToShare.itemType,
        itemToShare.itemId,
        itemToShare.currentVisibility,
        depsToShare,
        mode
      )
    }
        ...
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {(() => {
                // First filter by curriculum template compatibility
                const compatibleDepts = sharingInfo.cluster_departments.filter(dept => 
                  dept.curriculum_template === sharingInfo.curriculum_template
                )
                ...
                if (compatibleDepts.length === 0) {
                  return (
                    <div className="text-center py-4 text-gray-500 text-sm">
                      ...
                      <strong className="block mb-1">No compatible departments</strong>
                      <p className="text-xs">All other departments use a different curriculum template ({sharingInfo.curriculum_template === '2022' ? '2026' : '2022'}).</p>
                      <p className="text-xs mt-1">Only departments with template <strong>{sharingInfo.curriculum_template}</strong> can receive this content.</p>
                    </div>
                  )
                }
                ...
                if (filteredDepts.length === 0) {
                  return (
                    <div className="text-center py-4 text-gray-500 text-sm">
                      {sharingMode === 'add' 
                        ? 'All compatible departments already have this item' 
                        : sharingMode === 'remove'
                        ? 'No compatible departments have this item yet'
                        : 'No compatible departments available'}
                    </div>
                  )
                }
                ...
                return filteredDepts.map(dept => (
                  ...
                  >
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded ${ 
                      dept.curriculum_template === '2022' 
                        ? 'bg-amber-100 text-amber-800' 
                        : 'bg-emerald-100 text-emerald-800'
                    }`}>
                      {dept.curriculum_template}
                    </span>
                  </label>
                ))
              })()}
      ...
      <div className="card-custom p-6">
        ...
        <div className="space-y-4">
          {sharingInfo.normal_cards.map((card, index) => {
            ...
            const cardIsShared = card.visibility === 'CLUSTER'
            const cardIsReceived = cardIsShared && !cardIsOwner
              ...
              <div key={card.id} className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-semibold text-gray-900">{getCardDisplayName(card)}</span>
                    {card.courses && (
                      <span className="text-xs text-gray-500">({card.courses.length} courses)</span>
                    )}
                  </div>
                  
                  <div className="flex items-center space-x-2">
                    {/* Modify button for cards */}
                    {cardIsOwner && cardIsShared && (
                      <button
                        onClick={() => handleModifySharing('semester', card.id)}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-100 text-blue-700 hover:bg-blue-200 transition-all"
                        title="Modify sharing list (add/remove departments)"
                      >
                        Modify
                    ...
                    {/* Toggle visibility button */}
                    <button
                    onClick={() => cardIsOwner && handleToggleVisibility('semester', card.id, card.visibility)}
                    disabled={!cardIsOwner}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                      cardIsReceived
                        ? 'bg-blue-100 text-blue-700 cursor-not-allowed'
                        : cardIsShared
  ...
  const renderHonourCardsList = () => {
    if (!sharingInfo || !sharingInfo.honour_cards || sharingInfo.honour_cards.length === 0) {
      return null // Don't show section if no honour cards
    }

    return (
      <div className="card-custom p-6">
        <h3 className="text-md font-bold text-gray-900 mb-4">Honour Programme Cards</h3>
        <div className="space-y-3">
          {sharingInfo.honour_cards.map((honourCard) => {
            const cardIsOwner = honourCard.is_owner !== false
            const cardIsShared = honourCard.visibility === 'CLUSTER'
            const cardIsReceived = cardIsShared && !cardIsOwner

            return (
              <div key={honourCard.id} className="border border-purple-200 rounded-lg overflow-hidden bg-purple-50">
                <div className="flex items-center justify-between p-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <span className="font-semibold text-gray-900">{honourCard.title}</span>
                      {honourCard.semester_number && (
                        <span className="ml-2 text-xs text-gray-500">(Semester {honourCard.semester_number})</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    {cardIsOwner && cardIsShared && (
                      <button
                        onClick={() => handleModifySharing('honour_card', honourCard.id)}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-purple-200 text-purple-800 hover:bg-purple-300 transition-all"
                        title="Modify sharing list (add/remove departments)"
                      >
                        Modify
                      </button>
                    ...
                    <button
                      onClick={() => cardIsOwner && handleToggleVisibility('honour_card', honourCard.id, honourCard.visibility)}
                      disabled={!cardIsOwner}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                        cardIsReceived
                          ? 'bg-blue-100 text-blue-700 cursor-not-allowed'
      ...
      <div className="card-custom p-6 mt-6">
        ...
        {clusterContent.departments.map(dept => {
          const hasSharedContent = 
            ...
            (dept.honour_cards && dept.honour_cards.length > 0)
          ...
          return (
            <div key={dept.department_id} className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
              ...
              {dept.semesters && dept.semesters.length > 0 && (
                <div className="mt-4">
                  <p className="text-xs font-semibold text-gray-600 mb-2">Shared Curriculum Cards</p>
                  <div className="space-y-2">
                    {dept.semesters.map(card => {
                      ...
                      return (
                        <div key={card.id} className="bg-white rounded p-3 border border-green-200">
              ...
              {dept.honour_cards && dept.honour_cards.length > 0 && (
                <div className="mt-4">
                  <p className="text-xs font-semibold text-gray-600 mb-2">Shared Honour Programme Cards</p>
                  <div className="space-y-2">
                    {dept.honour_cards.map(honourCard => (
                      <div key={honourCard.id} className="bg-purple-50 rounded p-3 border border-purple-200">
