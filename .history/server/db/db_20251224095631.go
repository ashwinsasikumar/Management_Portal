...
func CreateNormalizedSyllabusTables() error {
	...
	// Migrate course_teamwork table to add activities column if it doesn't exist
	_, err := DB.Exec(`
		ALTER TABLE course_teamwork 
		ADD COLUMN IF NOT EXISTS activities JSON
	`)
	if err != nil {
		// Try MySQL 5.7 compatible version (check if column exists first)
		var columnExists int
		checkErr := DB.QueryRow(`
			SELECT COUNT(*) 
			FROM INFORMATION_SCHEMA.COLUMNS 
			WHERE TABLE_SCHEMA = DATABASE() 
			AND TABLE_NAME = 'course_teamwork' 
			AND COLUMN_NAME = 'activities'
		`).Scan(&columnExists)
		
		if checkErr == nil && columnExists == 0 {
			_, err = DB.Exec("ALTER TABLE course_teamwork ADD COLUMN activities JSON")
			if err != nil {
				log.Println("Error adding activities column to course_teamwork:", err)
				return err
			}
			fmt.Println("Added activities column to course_teamwork table")
		}
	}
