...
// AddSemesterNameColumn adds a name column to semesters table
func AddSemesterNameColumn() error {
	fmt.Println("Adding name and card_type columns to semesters table...")

	// Add name column to semesters
	if err := ensureColumnExists("semesters", "name", "VARCHAR(255) DEFAULT NULL"); err != nil {
		return fmt.Errorf("failed to add name to semesters: %w", err)
	}

	// Add card_type column to semesters
	if err := ensureColumnExists("semesters", "card_type", "VARCHAR(50) DEFAULT 'semester'"); err != nil {
		return fmt.Errorf("failed to add card_type to semesters: %w", err)
	}

	// Modify semester_number to allow NULL
	modifyQuery := "ALTER TABLE semesters MODIFY COLUMN semester_number INT NULL"
