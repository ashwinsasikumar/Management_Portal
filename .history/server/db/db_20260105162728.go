...
// CreateNormalCardsTable creates the normal_cards table for curriculum structure
func CreateNormalCardsTable() error {
	fmt.Println("Creating normal_cards table...")
	...
	query := `
		CREATE TABLE IF NOT EXISTS normal_cards (
			...
			card_type VARCHAR(50) NOT NULL DEFAULT 'semester',
			visibility ENUM('UNIQUE', 'CLUSTER') DEFAULT 'UNIQUE',
	...
	if _, err := DB.Exec(query); err != nil {
		return fmt.Errorf("failed to create normal_cards table: %w", err)
	}

	fmt.Println("normal_cards table created successfully!")
	return nil
}
