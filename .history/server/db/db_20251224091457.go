...
// CreateCoursesTable creates the core courses table
func CreateCoursesTable() error {
	...
	if err == nil && tableExists {
		...
		err = DB.QueryRow("SELECT COUNT(*) > 0 FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'courses' AND column_name = 'id'").Scan(&idColumnExists)
		
		if !idColumnExists {
			log.Println("WARNING: courses table exists but is missing 'id' column - needs recreation")
			log.Println("NOTE: Dropping dependent tables first to handle foreign key constraints")
			
			// Drop dependent tables that reference courses
			_, _ = DB.Exec("DROP TABLE IF EXISTS co_po_mapping")
			_, _ = DB.Exec("DROP TABLE IF EXISTS curriculum_courses")
			_, _ = DB.Exec("DROP TABLE IF EXISTS course_syllabus")
			_, _ = DB.Exec("DROP TABLE IF EXISTS syllabus_topics")
			_, _ = DB.Exec("DROP TABLE IF EXISTS syllabus_titles")
			_, _ = DB.Exec("DROP TABLE IF EXISTS syllabus_models")
			
			// Now drop the courses table
			_, dropErr := DB.Exec("DROP TABLE IF EXISTS courses")
			if dropErr != nil {
				log.Println("Error dropping courses table:", dropErr)
				return dropErr
			}
	...
	// Create table with appropriate syntax based on whether it needs recreation
	var query string
	if needsRecreation || !tableExists {
		// Use CREATE TABLE (without IF NOT EXISTS) when we know table doesn't exist
		query = `
		CREATE TABLE courses (
			...
			cia_marks INT DEFAULT 0,
			see_marks INT DEFAULT 0,
		...
		// Use CREATE TABLE IF NOT EXISTS for normal creation
		query = `
	...
	// Verify table was created with id column
	var hasID bool
	err = DB.QueryRow("SELECT COUNT(*) > 0 FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'courses' AND column_name = 'id'").Scan(&hasID)
	if err != nil || !hasID {
		log.Fatal("CRITICAL: courses table created but 'id' column not found!")
		return fmt.Errorf("courses table validation failed")
	}

	// Ensure all required columns exist (for legacy schemas)
	...
	_ = ensureColumnExists("courses", "credit", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "theory_hours", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "activity_hours", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "lecture_hours", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "tutorial_hours", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "practical_hours", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "cia_marks", "INT DEFAULT 0")
	_ = ensureColumnExists("courses", "see_marks", "INT DEFAULT 0")
