...
// CreateClusterTables creates tables for cluster management
func CreateClusterTables() error {
	tables := []string{
		...
		`CREATE TABLE IF NOT EXISTS cluster_mission (
			id INT AUTO_INCREMENT PRIMARY KEY,
			cluster_id INT NOT NULL,
			mission_text TEXT NOT NULL,
			position INT NOT NULL,
			UNIQUE KEY unique_cluster_position (cluster_id, position),
			FOREIGN KEY (cluster_id) REFERENCES clusters(id) ON DELETE CASCADE
		)`,
		`CREATE TABLE IF NOT EXISTS cluster_peos (
			id INT AUTO_INCREMENT PRIMARY KEY,
			cluster_id INT NOT NULL,
		...
		`CREATE TABLE IF NOT EXISTS cluster_pos (
			id INT AUTO_INCREMENT PRIMARY KEY,
			cluster_id INT NOT NULL,
		...
		`CREATE TABLE IF NOT EXISTS cluster_psos (
			id INT AUTO_INCREMENT PRIMARY KEY,
			cluster_id INT NOT NULL,
	...
	for _, createSQL := range tables {
		if _, err := DB.Exec(createSQL); err != nil {
			log.Println("Error creating cluster table:", err)
			return err
		}
	}
