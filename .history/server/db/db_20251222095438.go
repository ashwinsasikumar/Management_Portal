...
func CreateCourseSyllabusTable() error {
	...
	_ = dropColumnIfExists("course_syllabus", "textbooks")
	_ = dropColumnIfExists("course_syllabus", "unit1")
	_ = dropColumnIfExists("course_syllabus", "unit2")
	_ = dropColumnIfExists("course_syllabus", "unit3")
	_ = dropColumnIfExists("course_syllabus", "unit4")
	_ = dropColumnIfExists("course_syllabus", "unit5")
...
func CreateSyllabusRelationalTables() error {
	...
	if _, err := DB.Exec(`
		CREATE TABLE IF NOT EXISTS syllabus_models (
			...
			CONSTRAINT fk_models_syllabus FOREIGN KEY (syllabus_id) REFERENCES course_syllabus(id) ON DELETE CASCADE
		) ENGINE=InnoDB`); err != nil {
		return err
	}
	// Ensure required columns exist for legacy schemas
	_ = ensureColumnExists("syllabus_models", "course_id", "INT")
	_ = ensureColumnExists("syllabus_models", "syllabus_id", "INT NOT NULL")
	...
	_, _ = DB.Exec("CREATE INDEX IF NOT EXISTS idx_models_syllabus ON syllabus_models(syllabus_id)")
	// titles
	if _, err := DB.Exec(`
...
// EnsureCoursesColumns verifies required columns on courses and adds computed total_hours if missing
func EnsureCoursesColumns() error {
	// Base numeric hour and mark columns
	...
	// Generated columns cannot use IF NOT EXISTS; add if missing
	if exists, err := columnExists("courses", "total_hours"); err == nil && !exists {
		q := "ALTER TABLE courses ADD COLUMN total_hours INT GENERATED ALWAYS AS (lecture_hours + tutorial_hours + practical_hours) STORED"
		if _, err := DB.Exec(q); err != nil {
			log.Println("Warning: could not add generated column total_hours:", err)
		} else {
			fmt.Println("Added generated column total_hours to courses")
		}
	}
	// total_marks may already exist; attempt to add if missing for consistency
	if exists, err := columnExists("courses", "total_marks"); err == nil && !exists {
		q := "ALTER TABLE courses ADD COLUMN total_marks INT GENERATED ALWAYS AS (cia_marks + see_marks) STORED"
		if _, err := DB.Exec(q); err != nil {
			log.Println("Warning: could not add generated column total_marks:", err)
		} else {
			fmt.Println("Added generated column total_marks to courses")
		}
	}
	return nil
}
