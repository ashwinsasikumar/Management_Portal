...
func main() {
	...
	err = db.CreateCurriculumTable()
	if err != nil {
		log.Fatal("Failed to create curriculum table:", err)
	}

	// Create curriculum logs table
	err = db.CreateCurriculumLogsTable()
	if err != nil {
		log.Fatal("Failed to create curriculum logs table:", err)
	}
	...
	// Relational syllabus tables
	if err := db.CreateSyllabusRelationalTables(); err != nil {
		log.Fatal("Failed to create relational syllabus tables:", err)
	}

	// OPTIONAL: Create normalized schema (3NF, zero JSON fields)
	// Uncomment the line below to create the fully normalized tables alongside existing schema
	// This provides a migration path to eliminate all JSON fields
	if err := db.CreateNormalizedSchema(); err != nil {
		log.Println("Note: Normalized schema creation encountered errors (this is expected if tables exist)")
	}
	...
	// Save header-only fields (objectives, outcomes, resources, prerequisites)
