...
func main() {
	...
	// Create courses table (core table)
	err = db.CreateCoursesTable()
	if err != nil {
		log.Fatal("Failed to create courses table:", err)
	}

	// Create curriculum_courses junction table
	err = db.CreateCurriculumCoursesTable()
	if err != nil {
	...
	// Create normalized syllabus tables
