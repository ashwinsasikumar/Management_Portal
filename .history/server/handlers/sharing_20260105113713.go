...
// fetchSemestersWithVisibility fetches normal_cards (semesters) with their visibility status and courses
func fetchSemestersWithVisibility(regulationID int) []map[string]interface{} {
	query := "SELECT id, semester_number, COALESCE(visibility, 'UNIQUE') as visibility, source_department_id FROM normal_cards WHERE regulation_id = ? ORDER BY semester_number"
