	...
	for rows.Next() {
		...
		if err := rows.Scan(&courseID, &courseCode, &courseName, &visibility); err == nil {
			courses = append(courses, map[string]interface{}{
				...
				"visibility":  visibility,
				"is_owner":    semesterIsOwned, // Course ownership follows semester ownershipface{}{}
	for rows.Next() {
		var courseID int
		var courseCode, courseName, visibility string
		if err := rows.Scan(&courseID, &courseCode, &courseName, &visibility); err == nil {
...
func updateSemesterVisibility(semesterID int, visibility string) error {
	...
	err := db.DB.QueryRow("SELECT regulation_id, semester_number FROM semesters WHERE id = ?", semesterID).Scan(&regulationID, &semesterNum)
	if err != nil {
		return err
	}
	...
	if visibility == "CLUSTER" {
		// Share semester - copy to other cluster departments
		if err := shareSemesterToCluster(deptID, regulationID, semesterID, semesterNum); err != nil {
...
func updateCourseVisibility(courseID int, visibility string) error {
	...
	if visibility == "CLUSTER" {
		// Share course - copy to other cluster departments
		if err := shareCourseToCluster(deptID, regulationID, courseID); err != nil {
...
// shareSemesterToCluster copies a semester to other cluster departments
func shareSemesterToCluster(sourceDeptID, sourceRegulationID, semesterID, semesterNum int) error {
	// Get cluster ID for this department
	var clusterID sql.NullInt64
	err := db.DB.QueryRow(`
		SELECT cluster_id FROM cluster_departments WHERE department_id = ?
...
// shareCourseToCluster copies a course to other cluster departments
func shareCourseToCluster(sourceDeptID, sourceRegulationID, courseID int) error {
	// Get cluster ID for this department
	var clusterID sql.NullInt64
	err := db.DB.QueryRow(`
		SELECT cluster_id FROM cluster_departments WHERE department_id = ?
