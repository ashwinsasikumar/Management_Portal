...
func GetSemesters(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id, regulation_id, semester_number FROM semesters WHERE regulation_id = ? ORDER BY semester_number"
	...
	for rows.Next() {
		...
		err := rows.Scan(&sem.ID, &sem.RegulationID, &sem.SemesterNumber)
		if err != nil {
...
func CreateSemester(w http.ResponseWriter, r *http.Request) {
	...
	query := "INSERT INTO semesters (regulation_id, semester_number) VALUES (?, ?)"
	result, err := db.DB.Exec(query, sem.RegulationID, sem.SemesterNumber)
	if err != nil {
	...
	// Log the activity
	LogCurriculumActivity(regulationID, "Semester Added",
		"Added Semester "+strconv.Itoa(sem.SemesterNumber), "System")
...
func GetSemesterCourses(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		SELECT c.id, c.course_code, c.course_name, c.course_type, c.category, c.credit, 
			   ...
			   (c.lecture_hours + c.tutorial_hours + c.practical_hours) AS total_hours,
	...
	for rows.Next() {
		...
		err := rows.Scan(&course.ID, &course.CourseCode, &course.CourseName, &course.CourseType, &course.Category, &course.Credit,
			&course.LectureHours, &course.TutorialHours, &course.PracticalHours, &course.TotalHours,
			&course.CIAMarks, &course.SEEMarks, &course.TotalMarks, &course.RegCourseID)
		if err != nil {
...
func AddCourseToSemester(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		...
		result, err := db.DB.Exec(insertCourseQuery, course.CourseCode, course.CourseName, course.CourseType, course.Category, course.Credit,
			course.LectureHours, course.TutorialHours, course.PracticalHours, course.CIAMarks, course.SEEMarks)
		if err != nil {
	...
	// Fetch the complete course details including computed totals
	...
	err = db.DB.QueryRow(fetchQuery, courseID).Scan(&fullCourse.ID, &fullCourse.CourseCode, &fullCourse.CourseName,
		&fullCourse.CourseType, &fullCourse.Category, &fullCourse.Credit,
		&fullCourse.LectureHours, &fullCourse.TutorialHours, &fullCourse.PracticalHours,
		&fullCourse.TotalHours, &fullCourse.CIAMarks, &fullCourse.SEEMarks, &fullCourse.TotalMarks)
	if err != nil {
