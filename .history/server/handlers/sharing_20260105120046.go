...
func GetDepartmentSharingInfo(w http.ResponseWriter, r *http.Request) {
	...
	response["honour_cards"] = fetchHonourCardsWithVisibility(regulationID)
...
func fetchSemestersWithVisibility(regulationID int) []map[string]interface{} {
	query := "SELECT id, semester_number, COALESCE(card_type, 'semester') as card_type, COALESCE(name, '') as name, COALESCE(visibility, 'UNIQUE') as visibility, source_department_id FROM normal_cards WHERE regulation_id = ? ORDER BY semester_number, id"
	...
	for rows.Next() {
		...
		var cardType, name, visibility string
		var sourceDeptID sql.NullInt64
		if err := rows.Scan(&id, &semNum, &cardType, &name, &visibility, &sourceDeptID); err == nil {
...
func updateSemesterVisibility(semesterID int, visibility string, targetDepartments []int, mode string) error {
	// Get semester/card and regulation info including card_type and name
	var regulationID int
	var semesterNum sql.NullInt64
	var cardType, name sql.NullString
	var sourceDeptID sql.NullInt64
	err := db.DB.QueryRow("SELECT regulation_id, semester_number, COALESCE(card_type, 'semester'), COALESCE(name, ''), source_department_id FROM normal_cards WHERE id = ?", semesterID).Scan(&regulationID, &semesterNum, &cardType, &name, &sourceDeptID)
	if err != nil {
		return err
	}
	...
	if visibility == "CLUSTER" {
		...
		case "add":
			...
			if err := addDepartmentsToSemesterSharing(deptID, regulationID, semesterID, semNum, cType, cName, targetDepartments); err != nil {
		...
		default: // "replace" or empty
			...
			if err := shareSemesterToCluster(deptID, regulationID, semesterID, semNum, cType, cName, targetDepartments); err != nil {
...
func shareSemesterToCluster(sourceDeptID, sourceRegulationID, semesterID, semesterNum int, cardType, cardName string, targetDepartments []int) error {
		...
		if err == sql.ErrNoRows {
			...
			if semesterNum > 0 {
				result, err = db.DB.Exec(`
					INSERT INTO normal_cards (regulation_id, semester_number, card_type, name, visibility, source_department_id)
					VALUES (?, ?, ?, ?, 'CLUSTER', ?)
				`, targetRegulationID, semesterNum, cardType, cardName, sourceDeptID)
			} else {
				result, err = db.DB.Exec(`
					INSERT INTO normal_cards (regulation_id, card_type, name, visibility, source_department_id)
					VALUES (?, ?, ?, 'CLUSTER', ?)
				`, targetRegulationID, cardType, cardName, sourceDeptID)
			}
			if err != nil {
...
func fetchSharedSemesters(regulationID int) []map[string]interface{} {
	query := "SELECT id, COALESCE(semester_number, 0), COALESCE(card_type, 'semester'), COALESCE(name, '') FROM normal_cards WHERE regulation_id = ? AND visibility = 'CLUSTER' ORDER BY semester_number, id"
	rows, err := db.DB.Query(query, regulationID)
	if err != nil {
		return []map[string]interface{}{}
	}
...
func addDepartmentsToSemesterSharing(sourceDeptID, sourceRegulationID, semesterID, semesterNum int, cardType, cardName string, newDepartments []int) error {
	...
	return shareSemesterToCluster(sourceDeptID, sourceRegulationID, semesterID, semesterNum, cardType, cardName, newDepartments)
}
