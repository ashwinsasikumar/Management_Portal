...
func fetchTeamwork(courseID int) (*models.Teamwork, error) {
	...
	err := db.DB.QueryRow(`
		SELECT hours, activities 
		FROM course_teamwork 
		WHERE course_id = ?`, courseID).Scan(&hours, &activitiesJSON)
	...
	if len(activitiesJSON) > 0 {
		if err := json.Unmarshal(activitiesJSON, &activities); err != nil {
			log.Printf("Error unmarshaling teamwork activities: %v", err)
			activities = []string{}
		}
	}
...
func saveTeamwork(courseID int, teamwork *models.Teamwork) error {
	...
	activitiesJSON, err := json.Marshal(teamwork.Activities)
	if err != nil {
		log.Printf("ERROR: Failed to marshal activities to JSON: %v", err)
		return err
	}

	// Upsert teamwork with hours and activities
	result, err := db.DB.Exec(`
		INSERT INTO course_teamwork (course_id, hours, activities) 
		VALUES (?, ?, ?) 
		ON DUPLICATE KEY UPDATE hours = ?, activities = ?`,
		courseID, teamwork.Hours, activitiesJSON, teamwork.Hours, activitiesJSON)
	if err != nil {
		log.Printf("ERROR: Failed to insert/update course_teamwork: %v", err)
		return err
	}
	rowsAffected, _ := result.RowsAffected()
	log.Printf("DEBUG: course_teamwork insert/update affected %d rows", rowsAffected)
