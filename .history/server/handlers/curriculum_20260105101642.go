...
func GetSemesters(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id, regulation_id, semester_number, COALESCE(name, '') as name, COALESCE(card_type, 'semester') as card_type FROM semesters WHERE regulation_id = ? ORDER BY COALESCE(semester_number, 999), id"
...
func CreateSemester(w http.ResponseWriter, r *http.Request) {
	...
	query := "INSERT INTO semesters (regulation_id, semester_number, name, card_type) VALUES (?, ?, ?, ?)"
