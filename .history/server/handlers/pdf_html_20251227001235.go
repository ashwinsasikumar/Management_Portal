...
func fetchCompleteRegulationData(regulationID int) (*models.RegulationPDF, error) {
	...
	if err == nil {
		...
		// Mission is array of DepartmentListItem
		var missionItems []models.DepartmentListItem
		json.Unmarshal(missionJSON, &missionItems)
		pdfData.Overview.Mission = make([]models.DepartmentListItem, 0)
		for _, item := range missionItems {
			pdfData.Overview.Mission = append(pdfData.Overview.Mission, item)
		}
		
		// Parse PEOs, POs, PSOs and extract text
		...
		// Extract text from items for PDF (PEOs, POs, PSOs are []string in PDF model)
		pdfData.Overview.PEOs = make([]string, 0)
		for _, item := range peoItems {
			pdfData.Overview.PEOs = append(pdfData.Overview.PEOs, item.Text)
		}
		pdfData.Overview.POs = make([]string, 0)
		for _, item := range poItems {
			pdfData.Overview.POs = append(pdfData.Overview.POs, item.Text)
		}
		pdfData.Overview.PSOs = make([]string, 0)
		for _, item := range psoItems {
			pdfData.Overview.PSOs = append(pdfData.Overview.PSOs, item.Text)
		}
	}
