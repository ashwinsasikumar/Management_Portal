...
func GetRegulations(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id, name, academic_year, created_at FROM regulations ORDER BY created_at DESC"
	rows, err := db.DB.Query(query)
	if err != nil {
		log.Println("Error querying regulations:", err)
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(map[string]string{"error": "Failed to fetch regulations"})
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			log.Println("Error scanning regulation:", err)
			continue
		}
