...
// ensureSyllabusRecord ensures a syllabus record exists for the given course
func ensureSyllabusRecord(courseID int) (int, error) {
	var syllabusID int
	err := db.DB.QueryRow("SELECT id FROM course_syllabus WHERE course_id = ?", courseID).Scan(&syllabusID)

	if err == sql.ErrNoRows {
		result, err := db.DB.Exec("INSERT INTO course_syllabus (course_id) VALUES (?)", courseID)
		if err != nil {
			return 0, err
		}
