...
func GetCourseSyllabus(w http.ResponseWriter, r *http.Request) {
	...
	var outcomesJSON, referenceListJSON, prerequisitesJSON, teamworkJSON, selflearningJSON []byte

	err = db.DB.QueryRow(`
		 SELECT id, course_id, outcomes, reference_list, prerequisites, 
...
func SaveCourseSyllabus(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		...
		result, err := db.DB.Exec(`
			INSERT INTO course_syllabus (course_id, outcomes, reference_list, prerequisites, teamwork, selflearning)
			VALUES (?, ?, ?, ?, ?, ?)
		`, courseID, outcomesJSON, referenceListJSON, prerequisitesJSON, teamworkJSON, selflearningJSON)
		...
		_, err := db.DB.Exec(`
			...
			SET outcomes = ?, reference_list = ?, prerequisites = ?, teamwork = ?, selflearning = ?
			WHERE course_id = ?
		`, outcomesJSON, referenceListJSON, prerequisitesJSON, teamworkJSON, selflearningJSON, courseID)
