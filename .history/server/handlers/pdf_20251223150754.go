	...
	for semRows.Next() {
		...
		if courseRows != nil {
			...
			for courseRows.Next() {
				...
				course.Syllabus.Objectives, _ = fetchObjectives(course.ID)
				course.Syllabus.Outcomes, _ = fetchOutcomes(course.ID)
				course.Syllabus.ReferenceList, _ = fetchReferences(course.ID)
