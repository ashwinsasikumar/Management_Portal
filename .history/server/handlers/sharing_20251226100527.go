...
func removeDepartmentsFromSemesterSharing(sourceDeptID, semesterID int, departmentsToRemove []int) error {
	...
	for _, targetDeptID := range departmentsToRemove {
		...
		// Get all course IDs for this semester that need to be deleted
		rows, err := db.DB.Query(`
			SELECT course_id 
		...
		if err == nil {
			...
			for _, courseID := range courseIDs {
				...
				db.DB.Exec(`
					...
					INNER JOIN syllabus_titles stl ON st.title_id = stl.id
					INNER JOIN syllabus_models sm ON stl.model_id = sm.id
					INNER JOIN course_syllabus cs ON sm.syllabus_id = cs.id
					WHERE cs.course_id = ?
				`, courseID)

				// Delete syllabus titles
				db.DB.Exec(`
					DELETE stl FROM syllabus_titles stl
					INNER JOIN syllabus_models sm ON stl.model_id = sm.id
					INNER JOIN course_syllabus cs ON sm.syllabus_id = cs.id
					WHERE cs.course_id = ?
				`, courseID)

				// Delete syllabus models
				db.DB.Exec(`
					DELETE sm FROM syllabus_models sm
					INNER JOIN course_syllabus cs ON sm.syllabus_id = cs.id
					WHERE cs.course_id = ?
				`, courseID)
		...
		// Delete courses mapping from curriculum_courses
		_, err = db.DB.Exec(`
			DELETE FROM curriculum_courses 
			WHERE regulation_id = ? AND semester_id = ?
		`, targetRegulationID, copiedSemID)
		if err != nil {
			log.Printf("Error removing courses for semester %d: %v\n", copiedSemID, err)
		}
		...
		// Remove tracking records for all courses in this semester
		db.DB.Exec(`
			DELETE FROM sharing_tracking 
			WHERE source_dept_id = ? AND target_dept_id = ? 
			  AND item_type = 'course' 
			  AND source_item_id IN (
				  SELECT course_id FROM curriculum_courses 
				  WHERE regulation_id = ? AND semester_id = ?
			  )
		`, sourceDeptID, targetDeptID, targetRegulationID, copiedSemID)
	}
...
func removeDepartmentsFromCourseSharing(sourceDeptID, courseID int, departmentsToRemove []int) error {
	...
	for _, targetDeptID := range departmentsToRemove {
		...
		// Check if this course is still shared with other departments
		var shareCount int
		db.DB.QueryRow(`
			SELECT COUNT(*) FROM sharing_tracking 
			WHERE copied_item_id = ? AND item_type = 'course' AND source_dept_id = ?
		`, copiedCourseID, sourceDeptID).Scan(&shareCount)

		// If this was the only sharing instance, update visibility back to UNIQUE
		if shareCount == 1 {
			db.DB.Exec("UPDATE courses SET visibility = 'UNIQUE' WHERE course_id = ?", copiedCourseID)
			log.Printf("Updated course %d visibility to UNIQUE (no longer shared)\n", copiedCourseID)
		}
