...
// Header (from normalized tables) + Models (syllabus_models) → Titles (syllabus_titles) → Topics (syllabus_topics)
func GetCourseSyllabusNested(w http.ResponseWriter, r *http.Request) {
	...
	// 1. Ensure syllabus record exists
	...
	// 2. Fetch header data from normalized tables
	resp.Header.ID = syllabusID
...
func CreateModel(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		...
		result, err := db.DB.Exec(`
			INSERT INTO course_syllabus (course_id, outcomes, reference_list, prerequisites) 
			VALUES (?, JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY())`, courseID)
		if err != nil {
