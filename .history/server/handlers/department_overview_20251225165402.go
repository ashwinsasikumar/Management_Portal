		...
		if item.ID > 0 {
			...
			if existing.sourceDepartmentID.Valid && existing.sourceDepartmentID.Int64 != int64(departmentID) {
				...
				if textChanged {
					...
					log.Printf("Item %d from department %d is being modified - converting to owned item and unsharing original\n",
						item.ID, sourceDeptID)

					// First, find and unshare the original item in the source department
					// The original item has source_department_id = NULL and is in sourceDeptID
					...
					if err == nil {
						// Found the original item - unshare it
						...
						// Also update the original to UNIQUE visibility
					...
					// Update this item: make it owned by this department (source_department_id = NULL)
					// and set visibility to UNIQUE
					visibility := "UNIQUE"
