...
func unshareItemFromCluster(sourceDeptID, itemID int, tableName string) error {
	...
	`, sourceDeptID).Scan(&clusterID)
	if err != nil || !clusterID.Valid {
		// Department not in cluster, nothing to unshare
		return nil
	}
	...
	// Delete the shared item from each department
	// The item will have source_department_id = sourceDeptID
	deleteQuery := fmt.Sprintf(`
		DELETE FROM %s 
		WHERE department_id = ? AND source_department_id = ?
	`, tableName)
	...
	for rows.Next() {
		...
		// Delete shared item from this department
		_, err = db.DB.Exec(deleteQuery, targetDeptID, sourceDeptID)
		if err != nil {
			log.Printf("Error removing shared item from dept %d: %v\n", targetDeptID, err)
		}
	}
