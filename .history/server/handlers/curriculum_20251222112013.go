...
func GetSemesterCourses(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		SELECT c.id, c.course_code, c.course_name, c.course_type, c.category, c.credit, 
		       c.theory_hours, c.activity_hours, c.lecture_hours, c.tutorial_hours, c.practical_hours, c.cia_marks, c.see_marks, c.total_marks,
	...
	for rows.Next() {
		...
		err := rows.Scan(&course.ID, &course.CourseCode, &course.CourseName, &course.CourseType, &course.Category, &course.Credit,
			&course.TheoryHours, &course.ActivityHours, &course.LectureHours, &course.TutorialHours, &course.PracticalHours, &course.CIAMarks, &course.SEEMarks, &course.TotalMarks,
			&course.RegCourseID)
		if err != nil {
...
func AddCourseToSemester(w http.ResponseWriter, r *http.Request) {
	...
	fetchQuery := `SELECT id, course_code, course_name, course_type, category, credit, 
	               theory_hours, activity_hours, lecture_hours, tutorial_hours, practical_hours, cia_marks, see_marks, total_marks 
