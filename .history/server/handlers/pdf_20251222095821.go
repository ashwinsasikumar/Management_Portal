	...
	for semRows.Next() {
		...
		if courseRows != nil {
			...
			for courseRows.Next() {
				...
				courseRows.Scan(&course.ID, &course.CourseCode, &course.CourseName, &course.CourseType,
				...
				// Fetch syllabus
				var objJSON, outJSON, tbJSON, refJSON []byte
				err = db.DB.QueryRow(`
					SELECT objectives, outcomes, unit1, unit2, unit3, unit4, unit5, textbooks, reference_list
					FROM course_syllabus WHERE course_id = ?`, course.ID).
					Scan(&objJSON, &outJSON, &course.Syllabus.Unit1, &course.Syllabus.Unit2,
						&course.Syllabus.Unit3, &course.Syllabus.Unit4, &course.Syllabus.Unit5,
						&tbJSON, &refJSON)

				if err == nil {
					json.Unmarshal(objJSON, &course.Syllabus.Objectives)
					json.Unmarshal(outJSON, &course.Syllabus.Outcomes)
					json.Unmarshal(tbJSON, &course.Syllabus.Textbooks)
					json.Unmarshal(refJSON, &course.Syllabus.ReferenceList)
				}

				// Fetch CO-PO mapping
				course.COPOMapping = make(map[string]int)
				copoRows, _ := db.DB.Query("SELECT co_index, po_index, mapping_value FROM co_po_mapping WHERE course_id = ?", course.ID)
				if copoRows != nil {
