	...
	for semRows.Next() {
		...
		if courseRows != nil {
			...
			for courseRows.Next() {
				...
				// Fetch syllabus
				var outJSON, tbJSON, refJSON []byte
				err = db.DB.QueryRow(`
					SELECT outcomes, unit1, unit2, unit3, unit4, unit5, textbooks, reference_list
					FROM course_syllabus WHERE course_id = ?`, course.ID).
					Scan(&outJSON, &course.Syllabus.Unit1, &course.Syllabus.Unit2,
