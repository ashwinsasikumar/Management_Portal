...
func saveDepartmentList(departmentID int, tableName, columnName string, items []models.DepartmentListItem) error {
	...
	for i, item := range items {
		...
		if item.ID > 0 {
			...
			// Check if this is a shared item from another department
			if existing.sourceDepartmentID.Valid && existing.sourceDepartmentID.Int64 != int64(departmentID) {
				// This is a shared item from another department - can't modify it
				log.Printf("Skipping modification of shared item ID %d from department %d\n", item.ID, existing.sourceDepartmentID.Int64)
				continue
			}

			// Check if text or position changed
			textChanged := existing.text != item.Text
			
			if textChanged {
				// Text was modified
				// If item was CLUSTER, unshare it and make it UNIQUE
				if existing.visibility == "CLUSTER" {
					log.Printf("Item %d was modified and was shared - unsharing it\n", item.ID)
					// Unshare from cluster
