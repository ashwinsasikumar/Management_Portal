...
func UpdateItemVisibility(w http.ResponseWriter, r *http.Request) {
	...
	if reqData.ItemType == "semester" {
		if err := updateSemesterVisibility(reqData.ItemID, reqData.Visibility, reqData.TargetDepartments); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
	...
	if reqData.ItemType == "course" {
		if err := updateCourseVisibility(reqData.ItemID, reqData.Visibility, reqData.TargetDepartments); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
...
func copyCoursesBetweenSemesters(sourceRegID, sourceSemID, targetRegID, targetSemID int) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&courseID, &courseCode, &courseName, &credits, &courseType,
		...
		if err == sql.ErrNoRows {
			...
			result, err := db.DB.Exec(`
				...
				VALUES (?, ?, ?, ?, 'CLUSTER', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			`, courseCode, courseName, credits, courseType, category, credit,
