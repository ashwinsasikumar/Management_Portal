...
func GetSemesters(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id, regulation_id, semester_number, COALESCE(name, '') as name FROM semesters WHERE regulation_id = ? ORDER BY COALESCE(semester_number, 999), id"
	...
	for rows.Next() {
		...
		err := rows.Scan(&sem.ID, &sem.RegulationID, &semesterNum, &sem.Name)
		if err != nil {
...
func CreateSemester(w http.ResponseWriter, r *http.Request) {
	...
	query := "INSERT INTO semesters (regulation_id, semester_number, name) VALUES (?, ?, ?)"
	result, err := db.DB.Exec(query, sem.RegulationID, sem.SemesterNumber, sem.Name)
	if err != nil {
