...
func fetchCompleteRegulationData(regulationID int) (*models.RegulationPDF, error) {
	...
	if err == nil {
		...
		// Parse PEOs and POs (which are DepartmentListItem arrays)
...
func printToPDF(html string, res *[]byte) chromedp.Tasks {
	return chromedp.Tasks{
		...
		chromedp.ActionFunc(func(ctx context.Context) error {
			frameTree, err := chromedp.Run(ctx, chromedp.Evaluate(fmt.Sprintf(`
				document.open();
				document.write(%s);
				document.close();
		...
		chromedp.ActionFunc(func(ctx context.Context) error {
			buf, _, err := chromedp.Run(ctx, chromedp.PrintToPDF().
