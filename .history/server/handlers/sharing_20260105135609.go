...
func fetchSemestersWithVisibility(regulationID int) []map[string]interface{} {
	query := "SELECT id, semester_number, COALESCE(card_type, 'semester') as card_type, COALESCE(visibility, 'UNIQUE') as visibility, source_department_id FROM normal_cards WHERE regulation_id = ? ORDER BY semester_number, id"
