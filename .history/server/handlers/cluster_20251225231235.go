...
func GetAvailableDepartments(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		SELECT c.id, c.name, c.academic_year, c.regulation_no, 
		       COALESCE(do.id, 0) as dept_overview_id
		FROM curriculum c
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &name, &academicYear, &regulationNo, &deptOverviewID); err == nil {
			// Create department_overview entry if it doesn't exist
			if deptOverviewID == 0 {
				result, err := db.DB.Exec("INSERT INTO department_overview (regulation_id, vision) VALUES (?, '')", id)
				if err != nil {
					log.Printf("Error creating department_overview for curriculum %d: %v\n", id, err)
					continue
				}
				newID, _ := result.LastInsertId()
				deptOverviewID = int(newID)
			}
