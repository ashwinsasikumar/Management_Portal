...
func GetCourseSyllabusNested(w http.ResponseWriter, r *http.Request) {
	...
	var objectivesJSON, outcomesJSON, textbooksJSON, referenceJSON, prereqJSON, teamworkJSON, selflearningJSON []byte
	err = db.DB.QueryRow(`
		SELECT id, course_id, 
		       COALESCE(objectives, JSON_ARRAY()), 
		       COALESCE(outcomes, JSON_ARRAY()), 
		       COALESCE(textbooks, JSON_ARRAY()), 
		...
		Scan(&resp.Header.ID, &resp.Header.CourseID,
			&objectivesJSON, &outcomesJSON, &textbooksJSON, &referenceJSON, &prereqJSON, &teamworkJSON, &selflearningJSON)
	...
	json.Unmarshal(textbooksJSON, &resp.Header.Textbooks)
...
func CreateModel(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		...
		result, err := db.DB.Exec(`
			INSERT INTO course_syllabus (course_id, objectives, outcomes, textbooks, reference_list, prerequisites) 
			VALUES (?, JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY())`, courseID)
		if err != nil {
