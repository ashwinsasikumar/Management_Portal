...
func GetSemesters(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id, regulation_id, semester_number, COALESCE(name, '') as name, COALESCE(card_type, 'semester') as card_type FROM normal_cards WHERE regulation_id = ? ORDER BY COALESCE(semester_number, 999), id"
	...
	for rows.Next() {
		...
		err := rows.Scan(&sem.ID, &sem.RegulationID, &semesterNum, &sem.Name, &sem.CardType)
		if err != nil {
...
func CreateSemester(w http.ResponseWriter, r *http.Request) {
	...
	query := "INSERT INTO normal_cards (regulation_id, semester_number, name, card_type) VALUES (?, ?, ?, ?)"
	result, err := db.DB.Exec(query, sem.RegulationID, sem.SemesterNumber, sem.Name, sem.CardType)
	if err != nil {
	...
	if logName == "" {
		if sem.SemesterNumber != nil {
			logName = "Semester " + strconv.Itoa(*sem.SemesterNumber)
		} else {
			logName = "New Semester"
		}
	}
