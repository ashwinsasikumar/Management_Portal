...
func GetSemesters(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id, regulation_id, semester_number, COALESCE(name, '') as name FROM semesters WHERE regulation_id = ? ORDER BY semester_number"
	...
	for rows.Next() {
		...
		err := rows.Scan(&sem.ID, &sem.RegulationID, &sem.SemesterNumber, &sem.Name)
		if err != nil {
