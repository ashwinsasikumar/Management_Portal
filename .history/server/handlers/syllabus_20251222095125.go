...
func GetCourseSyllabus(w http.ResponseWriter, r *http.Request) {
	...
	var objectivesJSON, outcomesJSON, referenceListJSON, prerequisitesJSON, teamworkJSON, selflearningJSON []byte

	err = db.DB.QueryRow(`
		 SELECT id, course_id, objectives, outcomes, reference_list, prerequisites, 
...
func SaveCourseSyllabus(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		...
		result, err := db.DB.Exec(`
			INSERT INTO course_syllabus (course_id, objectives, outcomes, reference_list, prerequisites, teamwork, selflearning)
			VALUES (?, ?, ?, ?, ?, ?, ?)
		`, courseID, objectivesJSON, outcomesJSON, referenceListJSON, prerequisitesJSON, teamworkJSON, selflearningJSON)
		...
		_, err := db.DB.Exec(`
			...
			SET objectives = ?, outcomes = ?, reference_list = ?, prerequisites = ?, teamwork = ?, selflearning = ?
			WHERE course_id = ?
		`, objectivesJSON, outcomesJSON, referenceListJSON, prerequisitesJSON, teamworkJSON, selflearningJSON, courseID)
