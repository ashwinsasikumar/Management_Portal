...
func updateSemesterVisibility(semesterID int, visibility string, targetDepartments []int, mode string) error {
	...
	var sourceDeptID sql.NullInt64
	err := db.DB.QueryRow("SELECT regulation_id, semester_number, source_department_id FROM normal_cards WHERE id = ?", semesterID).Scan(&regulationID, &semesterNum, &sourceDeptID)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if err == sql.ErrNoRows {
			...
			result, err := db.DB.Exec(`
				INSERT INTO normal_cards (regulation_id, semester_number, visibility, source_department_id)
				VALUES (?, ?, 'CLUSTER', ?)
