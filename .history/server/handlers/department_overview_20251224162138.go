...
func saveDepartmentList(departmentID int, tableName, columnName string, items []models.DepartmentListItem) error {
	// Delete existing items for this department
	...
	// Insert new items with position and visibility
	insertQuery := fmt.Sprintf("INSERT INTO %s (department_id, %s, visibility, position) VALUES (?, ?, ?, ?)", tableName, columnName)
