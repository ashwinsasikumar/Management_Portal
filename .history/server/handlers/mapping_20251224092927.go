...
func GetCourseMapping(w http.ResponseWriter, r *http.Request) {
	...
	// Fetch COs from normalized course_outcomes table
	var cos []string
	outcomeRows, err := db.DB.Query("SELECT outcome_text FROM course_outcomes WHERE course_id = ? ORDER BY position", courseID)
	...
	if outcomeRows != nil {
		...
		for outcomeRows.Next() {
			var outcome string
			if err := outcomeRows.Scan(&outcome); err == nil {
				cos = append(cos, outcome)
			}
		}
	}
