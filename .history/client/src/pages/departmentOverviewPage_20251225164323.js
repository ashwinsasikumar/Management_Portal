...
function DepartmentOverviewPage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/regulation/${id}/overview`, {
        ...
        body: JSON.stringify({
          vision,
          mission: mission.filter(item => (typeof item === 'string' ? item.trim() : item.text?.trim()) !== ''),
          peos: peos.filter(item => (typeof item === 'string' ? item.trim() : item.text?.trim()) !== ''),
          pos: pos.filter(item => (typeof item === 'string' ? item.trim() : item.text?.trim()) !== ''),
          psos: psos.filter(item => (typeof item === 'string' ? item.trim() : item.text?.trim()) !== ''),
        }),
      })
      ...
      // Refresh data to get updated IDs and visibility
      fetchOverview()
    } catch (err) {
  ...
  const addItem = (setter, items) => {
    setter([...items, { text: '', visibility: 'UNIQUE' }])
  }
  ...
  const updateItem = (setter, items, index, value) => {
    ...
    if (typeof updated[index] === 'string') {
      updated[index] = { text: value, visibility: 'UNIQUE' }
    } else {
      // Update the text, preserve id and other properties
      ...
      <div className="space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item, index) => {
            ...
            const isShared = typeof item === 'object' && item.visibility === 'CLUSTER'
            // Only disable if it's from another department (source_department_id > 0)
            // Items you created will have source_department_id = undefined/null/0
            const isFromOtherDept = typeof item === 'object' && item.source_department_id && item.source_department_id > 0
              ...
              <div key={index} className="flex gap-3">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className={`input-custom w-full ${isShared && !isFromOtherDept ? 'border-green-300 bg-green-50' : ''} ${isFromOtherDept ? 'border-blue-300 bg-blue-50' : ''}`}
                    disabled={isFromOtherDept}
                    title={isFromOtherDept ? 'Shared from another department (read-only)' : isShared ? 'Shared with cluster - editing will make it private' : ''}
                  />
                  {isShared && !isFromOtherDept && (
                    <span className="absolute right-2 top-2 text-xs text-green-600 font-semibold">Shared - Edit to Unshare</span>
                  )}
                  {isFromOtherDept && (
                    <span className="absolute right-2 top-2 text-xs text-blue-600 font-semibold">From Cluster</span>
                  )}
                </div>
                <button
                  type="button"
                  onClick={() => removeItem(setter, items, index)}
                  className="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  title={isFromOtherDept ? "Cannot remove shared items from other departments" : "Remove item"}
                  disabled={isFromOtherDept}
                >
