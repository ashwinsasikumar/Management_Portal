...
function SharingManagementPage() {
  ...
  const handleToggleVisibility = async (itemType, itemId, currentVisibility, targetDepartments = null) => {
    ...
    try {
      ...
      if (selectedRegulation) {
        fetchSharingInfo(selectedRegulation.regulation_id)
      }
      if (selectedCluster) {
        fetchClusterContent(selectedCluster.id)
      }
      
      // Close department selector
  ...
  const selectAllDepartments = () => {
    if (sharingInfo?.cluster_departments) {
      setSelectedDepartments(sharingInfo.cluster_departments.map(d => d.department_id))
    }
  }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Select Departments to Share With</h3>
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {sharingInfo.cluster_departments.map(dept => (
                <label
  ...
  const handleToggleVisibilityOld = async (itemType, itemId, currentVisibility) => {
    const newVisibility = currentVisibility === 'UNIQUE' ? 'CLUSTER' : 'UNIQUE'
