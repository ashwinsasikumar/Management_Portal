          ...
          {sharingInfo.semesters.map((semester) => (
            <div key={semester.id} className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50">
                ...
                <button
                  onClick={() => handleToggleVisibility('semester', semester.id, semester.visibility)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                    semester.visibility === 'CLUSTER'
                      ? 'bg-green-100 text-green-700 hover:bg-green-200'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                  title={semester.visibility === 'CLUSTER' ? 'Shared with cluster' : 'Private to this department'}
                >
                  {semester.visibility === 'CLUSTER' ? (
                    <div className="flex items-center space-x-1">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="p-3 space-y-2">
                  {semester.courses.map((course) => (
                    <div key={course.id} className="flex items-center justify-between p-2 bg-white rounded border border-gray-100">
                      ...
                      <button
                        onClick={() => handleToggleVisibility('course', course.id, course.visibility)}
                        className={`ml-3 px-2 py-1 rounded text-xs font-semibold transition-all ${
                          course.visibility === 'CLUSTER'
                            ? 'bg-green-100 text-green-700 hover:bg-green-200'
                            : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                        }`}
                      >
                        {course.visibility === 'CLUSTER' ? 'Shared' : 'Private'}
                      </button>
                    </div>
                  ))}
                </div>
