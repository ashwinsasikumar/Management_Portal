...
function ClusterManagementPage() {
  ...
  const getDepartmentName = (deptId) => {
    const reg = regulations.find(r => r.id === deptId)
    return reg ? reg.name : `Department ${deptId}`
  }
                ...
                {clusterDepartments.map(dept => (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900">{getDepartmentName(dept.department_id)}</p>
                        <p className="text-xs text-gray-500">Department ID: {dept.department_id}</p>
                      </div>
                    </div>
                    <button
