...
function CurriculumPage() {
  ...
  const [newSemester, setNewSemester] = useState({ semester_num: '', name: '' })
  ...
  const handleCreateSemester = async (e) => {
    ...
    try {
      ...
      setNewSemester({ semester_num: '', name: '' })
          ...
          <div className="mb-6 bg-white/95 backdrop-blur-md p-6 rounded-2xl shadow-xl">
            <form onSubmit={handleCreateSemester} className="flex gap-4 items-end">
              <div className="flex-1">
                ...
                <input
                  type="number"
                  value={newSemester.semester_num}
                  onChange={(e) => setNewSemester({ ...newSemester, semester_num: e.target.value })}
                  placeholder="e.g., 1"
                  required
                  min="1"
              ...
              <div className="flex-1">
                <label className="block text-gray-700 font-semibold mb-2 text-sm">Semester Name</label>
                <input
                  type="text"
            ...
            {semesters.map(sem => (
              ...
              >
                <div className="text-center">
                  <div className="text-5xl font-bold text-indigo-600 mb-2">{sem.semester_num}</div>
                  <h3 className="text-lg font-semibold text-gray-800">{sem.name}</h3>
