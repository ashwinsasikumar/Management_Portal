...
function SharingManagementPage() {
  ...
  const getCardDisplayName = (card) => {
    ...
    if (cardType === 'vertical') {
      if (card.semester_number && card.name) return `Vertical ${card.semester_number}: ${card.name}`
      if (card.semester_number) return `Vertical ${card.semester_number}`
      return card.name || 'Vertical'
    }
              ...
              <div key={card.id} className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50">
                  <div className="flex items-center space-x-3">
                    <span className="text-sm font-bold text-gray-500 bg-gray-200 rounded-full w-7 h-7 flex items-center justify-center flex-shrink-0">
                      {index + 1}
                    </span>
                    ...
                    {dept.semesters.map(card => (
                      <div key={card.id} className="bg-white rounded p-3 border border-green-200">
                        <p className="text-sm font-semibold text-gray-900 mb-2">
                          {card.name || (card.semester_number ? `Semester ${card.semester_number}` : `Card ${card.id}`)}
                          {card.card_type && card.card_type !== 'semester' && (
                            <span className="ml-2 text-xs text-gray-500">({card.card_type})</span>
                          )}
                        </p>
                        {card.courses && card.courses.length > 0 && (
