...
function DepartmentOverviewPage() {
  ...
  const fetchOverview = async () => {
    try {
      ...
      // Extract text values from objects
      setMission((data.mission || []).map(item => typeof item === 'string' ? item : item.text))
      setPeos((data.peos || []).map(item => typeof item === 'string' ? item : item.text))
      setPos((data.pos || []).map(item => typeof item === 'string' ? item : item.text))
      setPsos((data.psos || []).map(item => typeof item === 'string' ? item : item.text))
      setError('')
    } catch (err) {
