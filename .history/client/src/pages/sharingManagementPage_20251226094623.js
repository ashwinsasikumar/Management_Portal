        ...
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {sharingInfo.cluster_departments
                .filter(dept => {
                  // Filter departments based on sharing mode
                  const isSharedWith = sharedWithDepartments.includes(dept.department_id)
                  
                  if (sharingMode === 'add') {
                    // For add mode, only show departments that DON'T have the item yet
                    return !isSharedWith
                  } else if (sharingMode === 'remove') {
                    // For remove mode, only show departments that HAVE the item
                    return isSharedWith
                  }
                  // For replace mode, show all departments
                  return true
                })
                .map(dept => (
                <label
