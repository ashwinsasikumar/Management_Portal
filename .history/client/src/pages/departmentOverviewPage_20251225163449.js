      ...
      <div className="space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item, index) => {
            ...
            const isFromOtherDept = typeof item === 'object' && item.source_department_id != null
              ...
              <div key={index} className="flex gap-3">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className={`input-custom w-full ${isShared ? 'border-green-300 bg-green-50' : ''} ${isFromOtherDept ? 'border-blue-300 bg-blue-50' : ''}`}
                    disabled={isFromOtherDept}
                    title={isFromOtherDept ? 'Shared from another department (read-only)' : isShared ? 'Shared with cluster' : ''}
                  />
                  {isShared && !isFromOtherDept && (
                    <span className="absolute right-2 top-2 text-xs text-green-600 font-semibold">Shared</span>
                  )}
                  {isFromOtherDept && (
